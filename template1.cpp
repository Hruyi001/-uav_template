...
std::pair<double, double> pixel_to_gps(int center_x, int center_y, const std::pair<int, int>& tile_index, const std::vector<std::vector<Tile>>& coordinates, const cv::Size& img_shape) {
    if (tile_index.first >= 0 && tile_index.second >= 0 && tile_index.first < coordinates.size() && tile_index.second < coordinates[tile_index.first].size()) {
        const auto& tile = coordinates[tile_index.first][tile_index.second];
        double tile_width_gps = tile.bottom_right.first - tile.top_left.first;
        double tile_height_gps = tile.top_left.second - tile.bottom_right.second;
        double gps_x = tile.top_left.first + (center_x / static_cast<double>(img_shape.width)) * tile_width_gps;
        double gps_y = tile.top_left.second - (center_y / static_cast<double>(img_shape.height)) * tile_height_gps;

       // std::cout << "Tile Index: (" << tile_index.first << ", " << tile_index.second << ")\n";
      //   std::cout << "Top Left: (" << tile.top_left.first << ", " << tile.top_left.second << ")\n";
      //  std::cout << "Bottom Right: (" << tile.bottom_right.first << ", " << tile.bottom_right.second << ")\n";
      // std::cout << "Tile Width GPS: " << tile_width_gps << "\n";
     //   std::cout << "Tile Height GPS: " << tile_height_gps << "\n";
      //  std::cout << "GPS X: " << gps_x << "\n";
      //  std::cout << "GPS Y: " << gps_y << "\n\n";
...
std::pair<double, double> template_demo(cv::Mat template_img, double angle, std::pair<double, double> last_gps) {
    ...
    // for (const auto& tpl_path : sorted_images) {
        ...
        if (tile_index.first != -1) {
            ...
            //cv::Mat img = cv::imread(tile_path);
            ...
            for (const auto& meth : methods) {
                cv::Mat res;
                if (img.type() != template_img.type()) {
                  std::cout << "图像类型错误" << std::endl;
                }
              ...
              //  cv::rectangle(img, top_left, bottom_right, cv::Scalar(255), 2);

              //  cv::imwrite("output/1.jpg", img);
                int center_x = (top_left.x + bottom_right.x) / 2;
                int center_y = (top_left.y + bottom_right.y) / 2;
                std::cout << center_x << "            "<<center_y << std::endl;
                auto center_gps = pixel_to_gps(center_x, center_y, tile_index, coordinates, img.size()); 
                last_gps = center_gps; 
                
                if (center_gps.first != -1) {
                    // std::cout << "中心GPS坐标：(" << center_gps.first << ", " << center_gps.second << ")" << std::endl;
                    std::cout.precision(7); // 设置精度为7
                    std::cout << "中心GPS坐标：("
                              << std::fixed << center_gps.first << ", "
                              << center_gps.second << ")"
                              << std::endl;
                }
